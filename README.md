...
Extracts annotations from JSON and populates database.

```bash
python -m src.process_image <image_id>
```

**Algorithm**:
1. Fetch JSON from T_IMAGES
2. Parse categories and annotations
3. Map category names to T_GARDINER_CODES
4. Insert glyphs into T_GLYPHES_RAW

---

#### src/sort.py
Reconstructs reading order from 2D bounding boxes.

```bash
# Preview mode
python -m src.sort <image_id> <tolerance> --preview

# Actually sort and save
python -m src.sort <image_id> <tolerance>

# Use center-of-gravity method
python -m src.sort <image_id> <tolerance> --method center
```

**Algorithm**:
1. Fetch all glyphs from T_GLYPHES_RAW
2. Sort by X coordinate
3. Group into columns using tolerance threshold
4. Within each column, sort by Y coordinate
5. If RTL, reverse column indices
6. Insert into T_GLYPHES_SORTED

**Parameters**:
- `tolerance`: X-axis distance threshold for column grouping (default: 100.0)
- `method`: "bbox" (top-left) or "center" (center of gravity)
- `preview`: Returns statistics without database insert

---

#### src/ngram.py
Finds frequently repeated n-glyph sequences.

```bash
python -m src.ngram
```

**Output**: CSV files (e.g., `top_4_grams.csv`) with columns:
- `ngram`: Tuple of glyph IDs
- `ngram_str`: Space-separated string
- `count`: Frequency
- `n`: N-gram length

---

#### src/visualize_columns.py
Creates visual overlay showing detected columns.

```bash
python -m src.visualize_columns <image_id> --method <bbox|center> --tolerance <value>
```

**Output**: PNG file with colored vertical lines marking column boundaries

---

#### src/suffixtree.py
Advanced pattern detection using suffix trees.
...
```
Loads credentials from `.env` and returns PostgreSQL connection.

#### src/database/select.py
```python
run_select(query: str, params: Sequence[Any]) → list[tuple]
```
Executes SELECT queries, returns all rows.

#### src/database/insert.py
```python
run_insert(query: str, params: Sequence[Any]) → int
```
Executes INSERT with `RETURNING id`, returns new row ID.

#### src/database/update.py
```python
run_update(query: str, params: Sequence[Any]) → int
```

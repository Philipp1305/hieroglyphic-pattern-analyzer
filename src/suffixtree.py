...
class SuffixTree:
		...
		for pos, ch in enumerate(text):
			...
			while remaining > 0:
				...
				if edge is None:
					...
					split = Node()
					active_node.add_edge(text[active_edge], Edge(edge.start, split_end, split))
	...
	def _collect(self, node: Node, path: List[Tuple[int, int]], min_length: int, results: List[Tuple[int, Tuple[int, ...], int]], sentinel_index: int) -> int:
		...
		for edge in node.children.values():
			...
			child_leaves = self._collect(edge.dest, path, min_length, results, sentinel_index)
	...
	def repeated_substrings(self, min_length: int = 1) -> List[Tuple[int, Tuple[int, ...], int]]:
		...
		for length, seq, count in results:
			if seq not in best or length > best[seq][0] or (length == best[seq][0] and count > best[seq][1]):
...
def main() -> None:
	parser = argparse.ArgumentParser(description="Suffix tree LCP finder for Gardiner codes")
	parser.add_argument("--min-length", type=int, default=1, help="Minimum pattern length to report")
	parser.add_argument("--limit", type=int, default=15, help="How many top patterns to show")
